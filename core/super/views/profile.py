...
class BaseProfileView(LoginRequiredMixin, TemplateView):
    """Vista base para el perfil de usuario, con métodos comunes para manejar los formularios."""
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == 'update_info': return self._handle_info(request)
        if action == 'update_avatar': return self._handle_avatar(request)
        if action == 'update_password': return self._handle_password(request)
    ...
    def _handle_avatar(self, request):
        ...
        if form.is_valid():
            ...
            messages.error(request, 'Error al subir la imagen. Verifica el formato.')
        return redirect(self.success_url_name)
    ...
    def _handle_password(self, request):
        ...
        if form.is_valid():
            form.save()
            update_session_auth_hash(request, request.user) # Evita que se cierre la sesión
...
class CustomerProfileView(BaseProfileView):
    """Vista para clientes: actualizar datos personales y foto de perfil."""
...
class AdminProfileView(AdminRequiredMixin, BaseProfileView):
    """Vista para administradores: actualizar datos personales, foto de perfil y contraseña."""
