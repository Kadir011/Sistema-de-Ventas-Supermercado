...
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white&labelColor=0d1117)](https://supabase.com/)
...
![Supabase Storage](https://img.shields.io/badge/Supabase_S3_Storage-3ECF8E?style=flat-square&logo=supabase&logoColor=white&labelColor=0d1117)
...
- [Changelog](#changelog)
  - [v1.4.0 — Migration to Supabase](#v140--migration-to-supabase)
...
> Deployed on **Render** · PostgreSQL on **Supabase** · Media on **Supabase Storage (S3)** · Static files via **WhiteNoise**
...
│   SSL · Session Pooler  │  │   Exponential backoff   │
...
| Database | PostgreSQL (Supabase) | 15+ | Relational DB with SSL, via Session Pooler |
| AI | Google Gemini Flash | — | Contextual chatbot |
| Media | Supabase Storage | S3-compatible | Image storage in production |
...
# ── Supabase Storage (S3-compatible, production media) ────
SUPABASE_S3_BUCKET=media
SUPABASE_S3_KEY_ID=your_access_key_id
SUPABASE_S3_SECRET=your_secret_access_key
SUPABASE_S3_URL=https://your-project-ref.supabase.co/storage/v1/s3
...
└── app/
    ├── config/
    │   ├── settings.py          # ATOMIC_REQUESTS=True, Supabase Storage (S3), WhiteNoise
    │   └── urls.py
    ├── core/
...
### v1.4.0 — Migration to Supabase
- Database moved from Neon to Supabase Postgres, connected through the Session Pooler for a stable persistent connection
- Media storage moved from Cloudinary to Supabase Storage using the S3-compatible protocol (`django-storages` + `boto3`)
- New environment variables: `SUPABASE_S3_BUCKET`, `SUPABASE_S3_KEY_ID`, `SUPABASE_S3_SECRET`, `SUPABASE_S3_URL`, `SUPABASE_S3_REGION`
- `DATABASES` config unchanged at the code level — only host/user/password values changed, since Supabase is also Postgres
...
<sub>Django 5.1.4 · Python 3.10+ · PostgreSQL (Supabase) · Supabase Storage · Render · v1.4.0</sub>
